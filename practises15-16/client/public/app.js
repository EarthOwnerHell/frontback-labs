  ...
  function loadNotes() {
    ...
    list.innerHTML = notes
      .map((note) => {
        ...
        const datetime =
          typeof note === "string" ? "" : note.datetime || note.timestamp || "";
        const dateLine = datetime
          ? `<div style="font-size: 0.8rem; opacity: 0.7;">${new Date(datetime).toLocaleString()}</div>`
          : "";
        return `<li class="card" style="margin-bottom: 0.5rem; padding: 0.5rem;">${text}${dateLine}</li>`;
      })
      .join("");
  }
